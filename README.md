...
The repository is implementing M2 Content Intelligence plus bounded M3 lexical, M4 project-graph, M5 rename-preview, M6 watch-reconciliation, and M8 product-UI slices while M0/M1 external evidence remains open. The CLI and Tauri desktop can initialize a local SQLite manifest, explicitly authorize an existing folder, run a metadata-only initial scan, persist progress, pause or resume safely, recover interrupted work, report graph statistics, and search current local paths and active extracted text. The Desktop interface is available in English and Traditional Chinese, detects a supported browser language on first launch, and persists only an explicit language selection locally. Rescans are idempotent in local tests; hard links share an identity, same-filesystem renames preserve identity, and symlinks and hidden entries are not followed.
