...
### Judge reproducibility and evidence boundary

DeskGraph is a pre-release development build, not a completed v0.1 release.
Use only a new synthetic folder and retain backups. The deterministic Rust suite
excludes exactly two opt-in macOS live tests whose FSEvents callback is
unavailable on this host; every other workspace test remains in the command:
...
The recorded local result is 452 deterministic Rust tests and 76 Vitest tests passing, but it
is not signed-package, clean-machine, 8 GB-memory, or cross-platform release
evidence. For the judge-facing walkthrough, current feature limits, dated
implementation evidence, and the 2:45 voiceover are in
[the Build Week submission pack](docs/HACKATHON_SUBMISSION.md) and
[demo script](docs/BUILD_WEEK_DEMO_SCRIPT.md).
