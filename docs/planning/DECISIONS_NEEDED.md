...
D-008 is resolved at the architecture level by ADR-024: native-first with a separately gated packaged fallback. D-015 now owns the fallback implementation choice. D-013 must be resolved before M4 introduces cross-root learned scoring; D-016 separately gates merge/split identity. D-014 must be resolved before splitting the long-running writer into a standalone OS service, but does not block an independent read-only stdio MCP process. D-017 must be accepted before Smart Cleanup exposes an executable system-trash action; it does not block explainable suggest-only candidates. GitHub remote ownership blocks only remote CI/Issue/Release evidence.
...
| D-017 | Cross-platform system-trash receipt, restore, and guarantee boundary | Before executable Smart Cleanup in M5/M6/M8 | Select exact macOS Trash, Windows Recycle Bin and Linux freedesktop Trash APIs/metadata; define per-volume locations, filename collisions, permissions, platform receipts, identity revalidation, restart recovery, Undo destination conflicts and externally emptied/changed items. Trash is an action-bound OS capability, never a scope: the adapter may act only on the immutable-plan-bound source, retain/verify only that action's opaque receipt and exact item identity, and cannot enumerate, search, index, or expose Trash paths/content. No shelling out, generic delete fallback, overwrite, permanent delete, or empty-trash capability. Linux remains experimental and cannot delay macOS/Windows | Suggestions only; no executable trash action. Undo may claim success only through the exact action receipt while the trashed item exists and identity matches; otherwise record `needs_attention` |
