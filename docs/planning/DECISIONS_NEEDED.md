...
| D-015 | Packaged OCR fallback provider                              | M2/M9                                 | Bake off Tesseract `eng`+`chi_tra` against PP-OCRv6 small/tiny on the same zh-Hant/English screenshot corpus, bytes-only boundary, process topology, CER/WER/spatial validity, latency/RSS/cancellation and checksums/SBOM; macOS arm64/Intel and Windows x64 block adoption, Linux remains experimental/nonblocking | No fallback runtime or router selected; native capability failures remain honest fixed errors |
