...
| Project/folder/related/duplicate/version graph | In progress        | Folder Profiles, durable correctable Project roots, and explicit ≤64 MiB full-byte exact-duplicate suggestions with immutable evidence/current verification pass locally with no model, membership or action; version/related/similarity, background duplicates, relation feedback, cross-root learning, evaluation and Project UI remain |
