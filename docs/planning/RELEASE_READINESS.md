...
| Project/folder/related/duplicate/version graph | In progress        | Bounded manifest-derived Folder Profile plus a durable stable-root candidate, immutable observations/signals, current-evidence validation, and append-only exact-root accept/reject correction pass locally with no model or membership edge; file membership, related/duplicate/version signals, cross-root learning, merge/split, evaluation and Project UI remain |
...
## M4 project-root local readiness note

The Folder Profile and exact-root correction slices are safe to exercise on a test folder after a completed scan: they read only current locations already inside the explicit manifest scope, stream a maximum of 100,000 descendants, fail without a partial profile on overflow, and perform no filesystem mutation or inference call. Direct project markers create a versioned, explainable `system_rule` observation; README alone is insufficient. Persisting a root re-derives current evidence, stores immutable observations/signals, and only an explicit user action appends `accepted` or `rejected`. Same-decision retries are idempotent, opposite decisions append corrections, and a rejected root stays rejected on later proposals. Explicit candidate responses may include the selected root path, while structured logs and the recent list omit root/member paths and names. Acceptance confirms only the root candidate and writes no `belongs_to` edge. This is not M4 or release completion: file membership, entities/topics, related/similar/duplicate/version relations, cross-root learning, merge/split, retrieval integration, Project UI, scale/memory and cross-platform evidence remain open.
