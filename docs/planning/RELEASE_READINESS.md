...
| Project/folder/related/duplicate/version graph | In progress        | Folder Profiles, durable correctable Project roots, and explicit ≤64 MiB full-byte exact-duplicate suggestions plus append-only exact-pair feedback/current revalidation pass locally with no model, membership or action; version/related/similarity, background duplicates, cross-pair learning, evaluation and Project UI remain |
...
The Folder Profile and exact-root correction slices are safe to exercise on a test folder after a completed scan: they use only current explicit-scope locations, immutable evidence and append-only user decisions, with no model, membership edge or file action. The exact-duplicate slice is safe only as an explicit development check for two canonical current non-empty files in the same scope: it denies symlink aliases and same identities, caps each source at 64 MiB, compares every byte through read-only handles, revalidates identity/metadata after reading, and appends fixed model-free evidence. An exact-pair accept/reject repeats that full live verification before appending immutable user feedback; same-decision retries are idempotent, later observations preserve the state, and the recent history is path-free and marked `verification_required`. Explicit duplicate/verify/decide responses intentionally include the two paths; structured logs omit paths, names, database location and content. This is not M4 or release completion: file membership, entities/topics, related/similarity/version relations, background duplicate discovery, larger-file hashing, cross-pair learning, merge/split, retrieval integration, Project UI, latency/RSS/8 GB and cross-platform evidence remain open.
