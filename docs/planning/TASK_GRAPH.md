...
- Current: ADR-016 and the durable untrusted-hint → per-scope debounce → stability/open-handle identity → atomically linked manifest reconcile core pass locally, including rename and two restart states; CLI/Desktop status is path-free and explicitly adapter-pending. The complete currently implemented Desktop surface also has typed in-bundle English/Traditional Chinese catalogs, safe first-launch locale detection, an always-available keyboard-accessible selector, explicit local preference persistence, and packaged macOS arm64 switch/restart evidence without a new dependency, network source or Tauri permission.
- Current fallback: the Tauri core owns a single in-process metadata-scan gate and deadline-driven coordinator; extraction/OCR cancellation and other durable engines do not acquire that mutex, although SQLite-level contention remains to be benchmarked. Only completed-scan scopes enter five-minute-interval polling. Each cycle schedules at most four due scopes, advances one full-scope reconciliation batch, preserves original due order for backlog-first fairness, and backs active-scan contention off for one second; deferred scopes are visible and active lookup is indexed. Durable active events resume after restart, failures remain path-free/degraded for the running coordinator until recovery, and shutdown stop waiting is bounded without claiming confirmed graceful termination. Next: connect an audited bounded native event adapter behind the same durable scan gate, add overflow/missed-event recovery, then replace routine full scans with efficient per-node reconciliation and incremental extraction/indexing. Add tray/autostart behavior and measured pause/battery/thermal/8 GB controls before claiming background Watch. Revisit a per-user writer daemon under D-014 only if UI-closed acceptance proves the current topology insufficient. Then add the separate suggest-only Smart Inbox state model and full onboarding/search/preview/history flows.
