...
- Next: expand the append-only state machine before implementing executor; add Move/cross-volume copy-verify-commit and platform system-trash actions, immediate pre-action identity validation, destination verification/hash or exact trash receipt/identity proof, process-kill/permission/conflict/disconnect/external-empty fault injection, startup recovery, rollback and idempotent Undo before any Desktop execution control. Resolve D-017 before exposing an executable trash action.
