...
- Current: ADR-018/ADR-019 provide bounded Folder Profiles and stable correctable Project roots. ADR-020 adds an explicit same-scope exact-duplicate suggestion only after canonical path, manifest/platform/open-handle identity, non-empty 64 MiB limit, full byte equality, post-read revalidation, immutable observation and log-redaction gates. ADR-021 adds append-only exact-pair accept/reject corrections; each decision repeats live byte verification, same decisions are idempotent, later observations retain the pair state, and recent history is path-free and marked `verification_required`. No result creates membership or a file action.
- Next: add deterministic version and related candidates with provenance/current-data invalidation, then define file-membership correction and evaluation. Background duplicate discovery and larger-file hashing need a separate bounded design. Resolve D-013 before cross-root learned scoring or merge/split; add retrieval filters and a backend-owned Project page only after those source-of-truth contracts pass.
