...
Three M2 vertical slices now run end to end. Explicit already-scanned text, Markdown, source-code, text-layer PDF, DOCX, PPTX, and XLSX files are resolved inside their authorized scope; the core revalidates the canonical scope, exclusion policy, manifest snapshot, and open-file identity; bounded providers receive only a controlled `Read + Seek`; durable SQLite jobs support cancellation and interrupted recovery; tagged byte/page/paragraph/slide/cell provenance and complete untrusted chunks publish atomically into SQLite and FTS. Invalid UTF-8, corrupt/encrypted/overlapping Office archives, unsafe names, unsupported compression, duplicate entries, inert macros/formulas/relationships/embeddings, XML structure/entity limits, corrupt/encrypted/active-content PDF, decompression/page/output/chunk limits, changed files, symlink swaps, cancellation, expired leases, false output sizes, and failed replacement are covered locally. M2 is not complete because image metadata, OCR, representative Office/PDF corpora, full cross-platform runtime evidence, and extraction benchmarks on documented 8 GB hardware remain open.
...
| M2 Content Intelligence       | In progress | Text/Markdown/code, bounded text-layer PDF, and allowlisted DOCX/PPTX/XLSX → durable job → open-file identity revalidation → structural provenance → atomic untrusted SQLite/FTS chunks → CLI/Desktop status | Audit and implement image metadata and OCR separately; add representative Office/PDF corpora and 8 GB benchmarks |
...
- The current 491-package lock was scanned against 1,160 cached RustSec advisories: zero known vulnerabilities, 16 unmaintained warnings, and one `glib` unsound warning. The 17 warnings exactly match the prior 488-package lock and trace to existing Tauri/Linux transitive paths; the three-package OOXML lock delta adds no advisory. R-010/R-016 remain open for upstream replacement, notices, SBOM, and release-platform review rather than a missing scan.
...
| Bounded resource policy | Verified locally for text, PDF, and OOXML providers | Defaults: 4 MiB source, 8 MiB decompression unit/stored output, 512 PDF pages, 2,048 chunks, 5 seconds; absolute caps: 64 MiB source/decompression/output, 4,096 pages, 65,536 chunks, 64 KiB/chunk, 60 seconds; Office also caps archive entries/selected parts, compression ratio, XML declarations/depth/events/text nodes, structural units, and shared strings; database independently validates output totals |
...
| Provenance, offsets, and untrusted classification | Verified locally for byte, PDF, and Office providers | Migration 0013 preserves existing byte/page rows and FTS, while Office chunks store paragraph, slide, or bounded Excel cell plus fragment with byte columns null; every chunk records source identity/snapshot, provider/version, ordinal, and fixed `untrusted_extracted_text` trust class |
| Per-file error isolation | Verified locally for text, PDF, and Office slices | Invalid UTF-8, corrupt/encrypted PDF, corrupt/encrypted/unsafe/overlapping Office archives, unsupported compression/XML, decompression/page/structure/output/chunk limits, and invalid policy produce fixed errors without aborting the process or publishing partial chunks |
