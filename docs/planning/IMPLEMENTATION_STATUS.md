...
Two M4 vertical slices now run end to end. An explicit already-scanned folder resolves to its current manifest identity, then streams at most 100,000 present descendant locations into deterministic direct/descendant counts, bytes, modified time, and file-category facts. Direct Cargo, JavaScript, Python, Go, Swift, Xcode, or Visual Studio markers create only an explainable `system_rule` Project Suggestion with basis-point confidence, provenance, completed-scan observation time, fixed provider/version, and no model; README alone never creates a suggestion. A Project root candidate can now persist by stable folder identity with immutable observations/signals and append-only explicit user accept/reject events. Same decisions are idempotent, opposite decisions append corrections, and a rejected root stays rejected on a later proposal. Explicit responses may return the selected root path; logs and recent summaries remain path-free. This is not M4 completion: file membership edges, entities/topics, related/similar/duplicate/version relations, clustering, cross-root learning, merge/split, retrieval filters, Project page, cross-platform runtime, and scale/memory evaluation remain open.
...
| M4 Project Graph              | In progress | Current manifest folder → bounded deterministic profile → explainable Project root candidate → immutable observations/signals → append-only user accept/reject correction → privacy-aware CLI; no file membership edge or model | Related/duplicate/version signals, file membership, cross-root learning, merge/split, evaluation, retrieval filters, Project page |
...
| Rust format, lint, and tests configured                  | Verified locally   | Rust 1.97.0; current workspace format and Clippy pass; 113 tests pass       |
...
For `prompts/05_PROJECT_GRAPH.md`, continue from ADR-018/ADR-019's derived Folder Profile and exact-root append-only correction without converting acceptance into automatic file membership. Add deterministic related/duplicate/version candidates with provenance and current-data invalidation next; resolve D-013 before cross-root learned scoring or merge/split, then add membership correction and evaluation before Project/folder retrieval filters or a Project page can claim source-of-truth behavior.
...
| Privacy-safe usable CLI | Verified locally | `folder profile`, `project propose/decide/status/list`; explicit profile/candidate responses may return the selected root path, while structured stderr and `project list` omit root/member paths and filenames |
...
- Evidence still open — current full-lock RustSec scan, Windows/macOS Intel/Linux runtime, 100k/8 GB/RSS evaluation, file membership edges, related/similar/duplicate/version relations, cross-root learning, merge/split, Project/folder retrieval filters, Project page and live Desktop interaction.
