...
One M4 read-only vertical slice now runs end to end. An explicit already-scanned folder resolves to its current manifest identity, then streams at most 100,000 present descendant locations into deterministic direct/descendant counts, bytes, modified time, and file-category facts. Direct Cargo, JavaScript, Python, Go, Swift, Xcode, or Visual Studio markers create only an explainable `system_rule` Project Suggestion with basis-point confidence, provenance, completed-scan observation time, fixed provider/version, and no model; README alone never creates a suggestion. CLI responses intentionally include only the selected folder path and aggregate/provenance facts while structured logs omit paths and names. This is not M4 completion: persisted Project nodes or membership edges, entities/topics, related/similar/duplicate/version relations, clustering, confirmation/reject/merge/split, correction feedback, retrieval filters, Project page, cross-platform runtime, and scale/memory evaluation remain open.
...
| M4 Project Graph              | In progress | Current manifest folder → bounded deterministic profile → marker-based explainable Project Suggestion → privacy-aware CLI; no membership edge or model | Persist project/edge candidates, related/duplicate/version signals, correction feedback, evaluation, Project page |
...
| Rust format, lint, and tests configured                  | Verified locally   | Rust 1.97.0; current workspace format and Clippy pass; 108 tests pass       |
...
For `prompts/05_PROJECT_GRAPH.md`, continue from ADR-018's derived Folder Profile without converting suggestions into automatic membership. Define durable Project/edge candidate and correction schemas first; then add related/duplicate/version signals, confirmation/reject/merge/split and scoring-feedback fixtures before Project/folder retrieval filters or a Project page can claim source-of-truth behavior.
...
## M4 folder-profile acceptance checklist
...
| Low-confidence behavior | Verified locally for this slice | Every heuristic result remains `project_suggestion`; no `belongs_to` edge, automatic membership, move, rename, or LLM action exists |
| Privacy-safe usable CLI | Verified locally | `folder profile --scope (--path|--node)` intentionally returns the selected folder path and aggregate/provenance facts; structured stderr omits folder/member paths and filenames |
| No new external dependency | Verified locally | Local `deskgraph-projects` crate reuses database/domain/scanner/test layers; no registry package, model, embedding runtime, API, or network client added |
| Persisted Project/edge candidates | Not started | Identity, candidate lifecycle, provenance persistence, low-confidence threshold and migration contracts required |
| Related, similarity, duplicate and version relations | Not started | Deterministic/hash/embedding signals, provenance, evaluation corpora and current-data invalidation required |
| User confirm/reject/merge/split and correction feedback | Not started | Correctable state model plus evidence that accepted/rejected memberships affect future scoring required |
...
| Scale and platform evidence | In progress | Small local macOS fixtures pass; 100k/8 GB/RSS and macOS Intel/Windows/Linux runtime remain open |
...
- Evidence still open — current full-lock RustSec scan, Windows/macOS Intel/Linux runtime, 100k/8 GB/RSS evaluation, persisted Project/edge candidate schemas, related/similar/duplicate/version relations, confirmation/reject/merge/split, correction feedback, retrieval filters, Project page and live Desktop interaction.
