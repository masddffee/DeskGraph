...
| M4 Project Graph              | In progress | Bounded Folder Profile → correctable stable Project root → explicit exact-byte duplicate plus feedback → explicit numeric filename-version suggestion → immutable provenance/path-free history; no file membership, model or action | Evidence-bound version feedback, related/general-version signals, background discovery, membership correction, cross-pair/root learning, evaluation, retrieval filters, Project page |
...
| Rust format, lint, and tests configured                  | Verified locally   | Rust 1.97.0; current workspace format and Clippy pass; 126 tests pass       |
...
- The local data volume now has only about 397 MiB free and repository `target` uses about 12 GB. The filename-version source state passed 126 Rust tests and Tauri release before a redundant rerun exhausted disk; two later nonessential edits were reverted. No cache was deleted without approval. Free at least 15 GB or explicitly authorize removal of `target/debug` before the next code slice; see `EXTERNAL_ACTIONS_REQUIRED.md`.
...
| Privacy-safe usable CLI | Verified locally | `folder profile`, `project propose/decide/status/list`, `relation duplicate/verify/decide/list/version/version-verify`; explicit profile/root and relation-detail responses may return selected paths, while structured stderr plus Project/relation lists omit paths, filenames, database paths and content |
...
| Explicit numeric filename-version candidates | Verified locally | ADR-022/migration 0011 share one NFC/lowercase parser across filesystem/database boundaries; allowlisted suffix, base/extension/direction, identity/current snapshot, stale/ambiguous input, migration preservation, immutable observation and privacy fixtures pass; candidate remains `suggested` |
...
| Evidence-bound version correction | Not started | Direction can change after rename, so exact-pair feedback cannot be reused; accepted ADR, observation-bound append-only events, idempotency/correction and future-suggestion fixtures are required before enabling decide |
...
- Evidence still open at the bounded exact-duplicate slice — current full-lock RustSec scan, Windows/macOS Intel/Linux runtime, duplicate latency/RSS and 8 GB evaluation, background discovery, larger-file strong hashing, related/similarity/version signals, file membership, cross-pair/root learning, merge/split, retrieval filters, Project page and live Desktop interaction. The following slice closes only exact-pair accept/reject correction.
...
- Evidence still open — current full-lock RustSec scan, Windows/macOS Intel/Linux runtime, duplicate latency/RSS and 8 GB evaluation, background discovery, larger-file strong hashing, deterministic related/version signals, file membership and correction, cross-pair/root learning, merge/split, retrieval filters, Project page and live Desktop interaction.
...
- Capacity note — after the successful 126-test/Tauri gates, a redundant test rerun following two temporary nonessential edits failed while building a Desktop archive because the data volume had about 397 MiB free. Those edits were reverted to the fully verified source state; cache deletion was not approved. Restore at least 15 GB before more code work.
- Evidence still open — evidence-bound directional version feedback, date/semantic/general version discovery, related/similarity relations, background discovery, evaluation corpora, file membership/correction, Windows/macOS Intel/Linux runtime, 8 GB/RSS, Project page, and live Desktop interaction.
