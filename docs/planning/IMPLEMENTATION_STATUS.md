...
Five M2 vertical slices now run end to end. Explicit already-scanned text, Markdown, source-code, text-layer PDF, DOCX, PPTX, XLSX, PNG, JPEG, GIF, WebP, BMP, and TIFF files are resolved inside their authorized scope; the core revalidates the canonical scope, exclusion policy, manifest snapshot, and open-file identity; bounded providers receive only a controlled `Read + Seek` or validated encoded bytes; durable SQLite jobs support cancellation and interrupted recovery. Text publishes atomically as tagged byte/page/paragraph/slide/cell/OCR-observation provenance plus complete untrusted SQLite/FTS chunks. Images publish signature-checked format and encoded dimensions into a separate structured table, with no pixel decode, EXIF, GPS, filename, or path. On macOS arm64, a separate PNG/JPEG Apple Vision provider requires `zh-Hant` and `en-US`, keeps normalized top-left boxes and confidence, and publishes only complete bounded OCR output. Invalid UTF-8, corrupt/encrypted/overlapping Office archives, unsafe names, unsupported compression, duplicate entries, inert macros/formulas/relationships/embeddings, XML structure/entity limits, corrupt/encrypted/active-content PDF, malformed/mismatched/oversized image headers, OCR format/source/pixel/output/observation/deadline limits, no-text output, changed files, symlink swaps, cancellation, expired leases, false output sizes, and failed replacement are covered locally. M2 is not complete because Windows native OCR, the packaged fallback, representative Office/PDF/image/OCR corpora, full cross-platform runtime evidence, actual native cancellation runtime evidence, and extraction benchmarks on documented 8 GB hardware remain open.
...
| M2 Content Intelligence       | In progress | Text/Markdown/code, bounded text-layer PDF, allowlisted DOCX/PPTX/XLSX, bounded image header metadata, and macOS PNG/JPEG Vision OCR → durable operation-specific job → open-file identity revalidation → atomic untrusted chunks or structured metadata → CLI/Desktop status | Separately gate Windows native OCR and the packaged fallback; add representative corpora, remote runtimes, native-cancel evidence, and 8 GB benchmarks |
...
| Rust format, lint, and tests configured                  | Verified locally   | Rust 1.97.0; current workspace format and Clippy pass; 166 tests pass       |
...
| Durable extraction jobs and cancellation | Verified locally | Operation-tagged queued/running/completed/failed/cancelled/interrupted states, durable cancel request, bounded polling, OCR atomic cancellation monitor, expiring runner lease, explicit resume after recovery; actual `VNRequest.cancel` runtime remains a native-platform gate |
...
| Bounded resource policy | Verified locally for text, PDF, OOXML, image-metadata, and macOS OCR providers | Defaults: 4 MiB general source, 8 MiB decompression unit/stored output, 512 PDF pages, 2,048 chunks, 5 seconds; generic absolute caps: 64 MiB source/decompression/output, 4,096 pages, 65,536 chunks, 64 KiB/chunk, 60 seconds. Office also caps archive/XML structure; image metadata caps source at 64 MiB, probe at 8 MiB absolute/2 MiB default, each dimension at 100,000, total encoded pixels at 500 million, and reader operations at 65,536. OCR is independently tighter at 32 MiB source, 16,384 per dimension, 64 Mi pixels, 8 MiB output, 4,096 observations, 256 KiB per observation, and 60 seconds; database independently validates output and spatial provenance. |
...
| Provenance, offsets, and untrusted classification | Verified locally for byte, PDF, Office, structured image metadata, and OCR | Migrations 0013–0014 preserve prior byte/page rows and add structural Office/image records. Migration 0015 preserves those rows and FTS while adding operation-tagged jobs plus OCR observation/fragment, normalized top-left box, confidence, source snapshot and provider provenance; OCR remains `untrusted_extracted_text` and receives no fabricated byte offset. |
| Per-file error isolation | Verified locally for text, PDF, Office, image, and macOS OCR slices | Invalid UTF-8, corrupt/encrypted PDF, corrupt/encrypted/unsafe/overlapping Office archives, unsupported compression/XML, malformed or extension-mismatched images, unsupported OCR formats, dimension/pixel/probe/decompression/page/structure/output/observation/chunk/deadline limits, provider failure, and invalid policy produce fixed errors without aborting the process or publishing partial output |
...
| Screenshot OCR with zh-TW and English | Verified locally for the macOS arm64 PNG/JPEG slice; M2 remains in progress | Exact target-specific `objc2-vision 0.3.2` provider receives only bounded bytes, requires `zh-Hant` plus `en-US`, and routes a durable `screenshot_ocr` job through spatial/confidence provenance to atomic SQLite/FTS. Mixed-language, no-text, corrupt/signature/format, source/pixel/output/observation/chunk/deadline, invalid provenance, source-change, migration preservation, durable cancel, privacy and real Vision→FTS smoke pass locally. Restricted sandbox failure is exposed; actual native cancel, macOS Intel, Windows, packaged fallback, corpus/RSS/8 GB, clean-machine and release evidence remain open. |

## Next handoff

Continue `prompts/03_EXTRACTORS_OCR.md` from the locally verified macOS Vision slice. Audit and implement the Windows bytes/raster boundary only after its exact binding/API/language-feature/runtime gates, then separately audit the packaged in-process Tesseract/Leptonica plus `eng`/`chi_tra` fallback; do not add a CLI subprocess or ambient model discovery. Add actual native cancellation, representative Office/PDF/image/OCR corpora and 8 GB residency evidence without relabeling local arm64 fixtures as cross-platform completion. In parallel, continue `prompts/04_HYBRID_SEARCH.md` from the verified lexical slice and bounded type/date/source filters: add project/folder filters only after their source-of-truth graph model exists, and extend the benchmark to 100k, representative corpora, peak RSS and remote platforms; vector/embedding candidates remain unselected. Keep M1 evidence closure as a parallel release workstream: Windows junction/hidden-attribute runtime fixtures, peak RSS on an unrestricted 8 GB machine, latest desktop interaction smoke, and remote macOS/Windows/Linux CI.
