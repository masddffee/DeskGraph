...
Four M2 vertical slices now run end to end. Explicit already-scanned text, Markdown, source-code, text-layer PDF, DOCX, PPTX, XLSX, PNG, JPEG, GIF, WebP, BMP, and TIFF files are resolved inside their authorized scope; the core revalidates the canonical scope, exclusion policy, manifest snapshot, and open-file identity; bounded providers receive only a controlled `Read + Seek`; durable SQLite jobs support cancellation and interrupted recovery. Text publishes atomically as tagged byte/page/paragraph/slide/cell provenance plus complete untrusted SQLite/FTS chunks. Images publish only signature-checked format and encoded dimensions into a separate structured table, with no pixel decode, EXIF, GPS, filename, or path. Invalid UTF-8, corrupt/encrypted/overlapping Office archives, unsafe names, unsupported compression, duplicate entries, inert macros/formulas/relationships/embeddings, XML structure/entity limits, corrupt/encrypted/active-content PDF, malformed/mismatched/oversized image headers, decompression/page/probe/dimension/output/chunk limits, changed files, symlink swaps, cancellation, expired leases, false output sizes, and failed replacement are covered locally. M2 is not complete because Screenshot OCR, representative Office/PDF/image corpora, full cross-platform runtime evidence, and extraction benchmarks on documented 8 GB hardware remain open.
...
| M2 Content Intelligence       | In progress | Text/Markdown/code, bounded text-layer PDF, allowlisted DOCX/PPTX/XLSX, and bounded image header metadata → durable job → open-file identity revalidation → atomic untrusted chunks or structured metadata → CLI/Desktop status; ADR-024 and the macOS binding/API/language spike resolve D-008 architecture | Implement the macOS Vision Screenshot OCR slice, then separately gate Windows/fallback; add representative corpora, remote runtimes, and 8 GB benchmarks |
...
| Rust format, lint, and tests configured                  | Verified locally   | Rust 1.97.0; current workspace format and Clippy pass; 149 tests pass       |
...
- The current 492-package lock was scanned against 1,160 cached RustSec advisories: zero known vulnerabilities, 16 unmaintained warnings, and one `glib` unsound warning. The 17 warnings exactly match the prior 491-package lock and trace to existing Tauri/Linux transitive paths; the one-package `imagesize` delta adds no advisory. R-010/R-016 remain open for upstream replacement, notices, SBOM, and release-platform review rather than a missing scan.
...
| Bounded resource policy | Verified locally for text, PDF, OOXML, and image-metadata providers | Defaults: 4 MiB general source, 8 MiB decompression unit/stored output, 512 PDF pages, 2,048 chunks, 5 seconds; absolute caps: 64 MiB source/decompression/output, 4,096 pages, 65,536 chunks, 64 KiB/chunk, 60 seconds. Office also caps archive/XML structure; image metadata caps source at 64 MiB, probe at 8 MiB absolute/2 MiB default, each dimension at 100,000, total encoded pixels at 500 million, and reader operations at 65,536; database independently validates output totals and dimensions. |
...
| Provenance, offsets, and untrusted classification | Verified locally for byte, PDF, Office, and structured image metadata | Migration 0013 preserves existing byte/page rows and FTS while Office stores structural chunk provenance. Migration 0014 preserves those rows and adds one active image-metadata record per node with source snapshot and provider provenance; image dimensions are not represented as extracted text. |
| Per-file error isolation | Verified locally for text, PDF, Office, and image slices | Invalid UTF-8, corrupt/encrypted PDF, corrupt/encrypted/unsafe/overlapping Office archives, unsupported compression/XML, malformed or extension-mismatched images, dimension/probe/decompression/page/structure/output/chunk limits, and invalid policy produce fixed errors without aborting the process or publishing partial output |
| Privacy-safe usable CLI | Verified locally | `extract start/create/run/status/list/cancel/resume/stats/image-metadata`; explicit `--path` resolves only an existing scanned node; binary tests prove path, filename, database path, and extracted text do not enter ordinary extraction or image-metadata stdout/stderr |
...
| Screenshot OCR with zh-TW and English | In progress | ADR-024 selects native-first plus separately gated packaged fallback. Exact minimal `objc2-vision 0.3.2` compiles; local arm64 runtime advertises `en-US`/`zh-Hant` and recognizes a controlled mixed fixture outside the restricted sandbox. No Repository provider, durable OCR job, SQLite provenance, Windows adapter, fallback, corpus, memory, or release evidence exists yet. |

## Next handoff

Continue `prompts/03_EXTRACTORS_OCR.md` from ADR-024 and the locally verified macOS binding/API/language spike. Implement the first path-free macOS Vision job through Manifest identity, bounded image validation, separate OCR provider, spatial provenance, atomic SQLite/FTS publication, cancellation and CLI status; keep Windows binding and packaged Tesseract fallback behind their own dependency/runtime gates. Add representative Office/PDF/image/OCR corpora and 8 GB residency evidence without relabeling local fixtures as cross-platform completion. In parallel, continue `prompts/04_HYBRID_SEARCH.md` from the verified lexical slice and bounded type/date/source filters: add project/folder filters only after their source-of-truth graph model exists, and extend the benchmark to 100k, representative corpora, peak RSS and remote platforms; vector/embedding candidates remain unselected. Keep M1 evidence closure as a parallel release workstream: Windows junction/hidden-attribute runtime fixtures, peak RSS on an unrestricted 8 GB machine, latest desktop interaction smoke, and remote macOS/Windows/Linux CI.

For `prompts/05_PROJECT_GRAPH.md`, continue from ADR-018 through ADR-023's Folder Profile, exact-root correction, bounded exact-duplicate observation/feedback, explicit numeric filename-version suggestion, and evidence-bound directional correction without converting any suggestion or acceptance into automatic membership or action. Next add deterministic related candidates with provenance/current-data invalidation. Keep background duplicate discovery/larger-file hashing separate, resolve D-013 before cross-pair/root learned scoring or merge/split, and require membership correction/evaluation before Project/folder retrieval filters or a Project page can claim source-of-truth behavior.
...
- Evidence still open — representative real-world Office corpus quality/latency, aggregate peak RSS on documented 8 GB hardware, native macOS Intel/Windows/Linux runtime, latest live Desktop interaction, image metadata, screenshot OCR, and extraction scheduling/resource policy. This slice is locally code-ready but DeskGraph remains **do not ship**.
...
- Evidence still open — representative image corpus quality/latency, native macOS Intel/Windows/Linux runtime, latest live Desktop interaction, aggregate peak RSS on documented 8 GB hardware, and Screenshot OCR. This slice is locally code-ready but DeskGraph remains **do not ship**.
