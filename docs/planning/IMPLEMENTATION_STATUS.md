...
| M2 Content Intelligence       | In progress | Text/Markdown/code, bounded text-layer PDF, allowlisted DOCX/PPTX/XLSX, and bounded image header metadata → durable job → open-file identity revalidation → atomic untrusted chunks or structured metadata → CLI/Desktop status | Resolve and implement D-008 Screenshot OCR; add representative document/image corpora, remote runtimes, and 8 GB benchmarks |
...
| Rust format, lint, and tests configured                  | Verified locally   | Rust 1.97.0; current workspace format and Clippy pass; 140 tests pass       |
