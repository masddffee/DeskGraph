...
| Rust format, lint, and tests configured                  | Verified locally   | Rust 1.97.0; current workspace format and Clippy pass; 197 tests pass       |
...
- The current 494-package lock was scanned against 1,160 cached RustSec advisories: zero known vulnerabilities, 16 unmaintained warnings, and one `glib` unsound warning. The 17 warnings exactly match the prior 492/493-package locks and trace to existing Tauri/Linux transitive paths; neither the target-specific `objc2-vision` package nor the workspace-only evaluator/direct `sha2` edge adds an advisory. R-010/R-016 remain open for upstream replacement, notices, SBOM, and release-platform review rather than a missing scan.
...
| Screenshot OCR with Traditional Chinese and English | macOS arm64 runtime verified locally; Windows provider code/cfg verified; evaluator contract verified locally; M2 in progress | Exact `objc2-vision 0.3.2` routes bounded bytes through spatial/confidence provenance to atomic SQLite/FTS and has a real mixed-language smoke. Reused Microsoft `windows 0.61.3` provider code covers identity preflight, owned bytes, requested/resolved language policy, nullable confidence, strict angle/boxes, deterministic de-duplication, terminal-close lifecycle, bounded caller return and single cleanup worker. The D-015 evaluator binds exact corpus/run bytes, separates completed/failed evidence and emits no OCR text/path. Host/evaluator tests and Windows cfg check/Clippy pass; real Windows/MSIX/language/OCR/cancel/cleanup/RSS, provider runners, representative corpus, fallback selection/implementation, 8 GB and release evidence remain open. |

## Next handoff

Continue `prompts/03_EXTRACTORS_OCR.md` from the macOS runtime slice, Windows code/cfg slice, and D-015 evaluation contract. Build provider-specific runners that emit the strict run contract, then run the same licensed representative corpus through native OCR, Tesseract and PP-OCRv6 before accepting a packaged fallback. Run the exact real Windows/MSIX identity, requested/resolved language, mixed/no-text/corrupt/limit, `TextAngle`, native cancellation/cleanup/single-worker recovery, atomic FTS and RSS fixtures listed in `EXTERNAL_ACTIONS_REQUIRED.md`; no fallback behavior is currently claimed and no dormant router should be added. Add representative Office/PDF/image/OCR corpora and 8 GB evidence without relabeling the synthetic evaluator fixture or compile fixtures as runtime completion. Continue `prompts/04_HYBRID_SEARCH.md` from the verified lexical slice: add project/folder filters only after their graph source of truth exists, then establish a versioned SQLite embedding/exact-search baseline and 100k/representative/RSS/remote evidence before selecting a disposable ANN accelerator. Keep Windows scanner fixtures, latest Desktop smoke, and remote CI as parallel evidence workstreams.
...
- Decision scope — the workspace-only `deskgraph-ocr-evaluator` is evidence tooling, not a shipped provider, user vertical slice, accepted fallback, or M2 completion. It reads only two explicit JSON inputs, never reads an image/path, writes no file, chooses no winner, and emits no expected/recognized text, raw command, image path, or user-file path.
- Reproducibility — the evaluator hashes the exact corpus and run bytes. Every run must cite the exact corpus digest, so changing ground truth, normalization, tags, or case order produces distinguishable evidence. Reports retain corpus/run digests, provider/runtime kind, OS/build, CPU/RAM, DeskGraph commit, harness/version, and only a command digest. OS-managed and packaged-model provenance are distinct; packaged runtimes require artifact and model-manifest checksums.
...
- Final local verification — 17 evaluator tests and 197 workspace Rust tests pass; workspace all-target/all-feature Clippy passes with `-D warnings`; the release CLI evaluates the checked-in examples and emits a zero-edit report with explicit missing evidence. `pnpm check` passes Prettier, ESLint, TypeScript, 19 Vitest tests and Vite production build; Tauri no-bundle release builds `target/release/deskgraph-desktop`. The evaluator also passes Windows x64 target check and Clippy after fetching the lock-pinned Windows-only terminal-style package; this is cfg/typecheck evidence, not Windows link/runtime.
- Evidence still open — licensed representative Traditional Chinese/English/no-text/difficulty corpus; native/Tesseract/PP-OCRv6 runners; actual runtime latency/RSS/cancellation/unload; macOS arm64/Intel, Windows x64 and nonblocking Linux packaging; spatial ground truth; winner thresholds and fallback implementation. D-015 and M2 remain **in progress**.
