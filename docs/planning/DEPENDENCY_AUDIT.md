...
The M4 Folder Profile slice adds only the local path-based `deskgraph-projects` workspace crate. The later Project root correction flow and exact-duplicate relation slice add no registry package: they promote the already-audited local scanner/identity layers to runtime use and rely on fixed-size Rust standard-library reads rather than a hashing library. `serde_json` and `tempfile` remain test-only. `Cargo.lock` still contains 488 packages; its only delta records the existing local identity dependency for `deskgraph-projects`. No graph library, embedding/vector runtime, hash package, model, API, network client, shell, Python, Docker, Ollama, or native runtime was added. ADR-018/ADR-019/ADR-020 require deterministic current manifest facts, immutable evidence, exact-root feedback, and bounded byte equality before any future dependency-backed similarity, larger-file hashing, or learned scoring signal.
