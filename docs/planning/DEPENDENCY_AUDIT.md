...
The M4 Folder Profile slice adds only the local path-based `deskgraph-projects` workspace crate. The later Project root correction, exact-duplicate relation, and exact-pair feedback flows add no registry package: they promote the already-audited local scanner/identity layers to runtime use and rely on SQLite plus fixed-size Rust standard-library reads rather than a hashing library. `serde_json` and `tempfile` remain test-only. `Cargo.lock` still contains 488 packages; its only delta records the existing local identity dependency for `deskgraph-projects`. No graph library, embedding/vector runtime, hash package, model, API, network client, shell, Python, Docker, Ollama, or native runtime was added. ADR-018 through ADR-021 require deterministic current manifest facts, immutable evidence, append-only exact-root/exact-pair feedback, and bounded byte equality before any future dependency-backed similarity, larger-file hashing, or learned scoring signal.
...
- The last complete all-target scan before PDF loaded 1,160 cached RustSec advisories and scanned 457 lockfile packages: zero known vulnerabilities plus 17 warnings—ten unmaintained GTK3 binding crates, `proc-macro-error`, five unmaintained `unic-*` crates, and one `glib` unsound advisory. The isolated 53-package PDF closure separately returned zero findings. A post-integration scan was requested at the 483-package PDF state but rejected because the local tool quota was exhausted; the current 488-package lock still requires that full scan, and the older all-target result must not be presented as current.
