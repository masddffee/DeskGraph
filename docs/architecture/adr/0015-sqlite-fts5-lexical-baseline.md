...
- Accept only normalized queries from 3 through 256 Unicode characters. Reject empty, shorter, longer, or control-character queries with fixed error codes. Do not silently run a linear-scan fallback for shorter terms.
