...
The current Project Graph boundary is defined by ADR-018 through ADR-022: derived Folder Profiles, immutable/correctable Project roots, bounded exact-byte duplicate candidates, append-only exact-pair feedback that requires live revalidation, and conservative explicit-numeric filename-version suggestions.
