...
- Tauri-owned five-minute-interval metadata polling fallback for previously scanned scopes, with four-scope scheduling and one-reconciliation-batch cycle limits, backlog-first fairness, one-second active-scan contention backoff, indexed active lookup, visible deferred-scope backpressure, deadline-driven idle sleep, race-free foreground wakeups, in-process path-free degraded state, bounded stop wait, live English/Traditional Chinese runtime status, and no automatic content extraction or file action.
